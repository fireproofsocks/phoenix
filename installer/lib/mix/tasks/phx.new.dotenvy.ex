defmodule Mix.Tasks.Phx.New.Dotenvy do
  @moduledoc """
  ...
  """
  use Mix.Task
  alias Phx.New.{Generator, Project, SingleDotenvy, Umbrella, Web, Ecto}
  ...
  def run(argv) do
    ...
    case OptionParser.parse!(argv, strict: @switches) do
      {_opts, []} ->
        Mix.Tasks.Help.run(["phx.new.dotenvy"])

      {opts, [base_path | _]} ->
        if opts[:umbrella] do
          generate(base_path, Umbrella, :project_path, opts)
        else
          generate(base_path, SingleDotenvy, :base_path, opts)
        end
    end
  end
